...
:apt-fast-bionic
START /MIN /WAIT "apt-fast" %GO% "DEBIAN_FRONTEND=noninteractive dpkg -i /tmp/%GITPRJ%/deb/aria2_1.35.0-1build1_amd64.deb /tmp/%GITPRJ%/deb/libaria2-0_1.35.0-1build1_amd64.deb /tmp/%GITPRJ%/deb/libssh2-1_1.8.0-2.1build1_amd64.deb /tmp/%GITPRJ%/deb/libc-ares2_1.15.0-1build1_amd64.deb ; chmod +x /tmp/%GITPRJ%/dist/usr/local/bin/apt-fast ; cp -p /tmp/%GITPRJ%/dist/usr/local/bin/apt-fast /usr/local/bin" > NUL
...
:apt-fast-jammy
echo %go%
%GO% "DEBIAN_FRONTEND=noninteractive apt-get install -y apt-fast"
%GO% "DEBIAN_FRONTEND=noninteractive apt --fix-broken install"
rem START /MIN /WAIT "apt-fast" %GO% "DEBIAN_FRONTEND=noninteractive dpkg -i /tmp/%GITPRJ%/deb/aria2_1.35.0-1build1_amd64.deb /tmp/%GITPRJ%/deb/libaria2-0_1.35.0-1build1_amd64.deb /tmp/%GITPRJ%/deb/libssh2-1_1.8.0-2.1build1_amd64.deb /tmp/%GITPRJ%/deb/libc-ares2_1.15.0-1build1_amd64.deb ; chmod +x /tmp/%GITPRJ%/dist/usr/local/bin/apt-fast ; cp -p /tmp/%GITPRJ%/dist/usr/local/bin/apt-fast /usr/local/bin" > NUL
...
ECHO [%TIME:~0,8%] Install Web Browser and CRD (~1m30s)
%GO% "DEBIAN_FRONTEND=noninteractive apt-fast -y install falkon seamonkey-mozilla-build vlc vlc-bin vlc-l10n vlc-plugin-notify vlc-plugin-qt vlc-plugin-samba vlc-plugin-skins2 vlc-plugin-video-splitter vlc-plugin-visualization --no-install-recommends ; update-alternatives --install /usr/bin/www-browser www-browser /usr/bin/seamonkey 100 ; update-alternatives --install /usr/bin/gnome-www-browser gnome-www-browser /usr/bin/seamonkey 100 ; update-alternatives --install /usr/bin/x-www-browser x-www-browser /usr/bin/seamonkey 100 ; cd /tmp/%GITPRJ%/deb ; wget -q https://dl.google.com/linux/direct/chrome-remote-desktop_current_amd64.deb ; dpkg -i /tmp/%GITPRJ%/deb/chrome-remote-desktop_current_amd64.deb" > ".\logs\%TIME:~0,2%%TIME:~3,2%%TIME:~6,2% Web Browser and CRD.log" 2>&1
%GO% "DEBIAN_FRONTEND=noninteractive apt-get upgrade"
...
%GO% "sed -i 's#SCHT#%SCHT%#g' /tmp/%GITPRJ%/dist/usr/local/bin/restartwsl ; sed -i 's#DISTRO#%DISTRO%#g' /tmp/%GITPRJ%/dist/usr/local/bin/restartwsl"
%GO% "sed -i 's/QQQ/%WINDPI%/g' /tmp/%GITPRJ%/dist/etc/skel/.config/kdeglobals"
%GO% "sed -i 's/QQQ/%LINDPI%/g' /tmp/%GITPRJ%/dist/etc/skel/.config/kcmfonts"
%GO% "sed -i 's/KPANEL/%KPANEL%/g' /tmp/%GITPRJ%/dist/etc/skel/.config/plasmashellrc"
...
%GO% "sed -i 's/port=3389/port=%RDPPRT%/g' /tmp/%GITPRJ%/dist/etc/xrdp/xrdp.ini ; cp /tmp/%GITPRJ%/dist/etc/xrdp/xrdp.ini /etc/xrdp/xrdp.ini"
...
%GO% "sed -i 's/WSLINSTANCENAME/%DISTRO%/g' /tmp/%GITPRJ%/dist/usr/local/bin/initwsl"
%GO% "sed -i 's/\\h/%DISTRO%/g' /tmp/%GITPRJ%/dist/etc/skel/.bashrc ; ln -s /usr/lib/x86_64-linux-gnu/libexec/kf5/kdesu /usr/bin/kdesu ; rm /usr/lib/x86_64-linux-gnu/qt5/plugins/discover/fwupd-backend.so"
...
%GO% "chmod 644 /tmp/%GITPRJ%/dist/etc/wsl.conf ; chmod 644 /tmp/%GITPRJ%/dist/var/lib/xrdp-pulseaudio-installer/*.so"
%GO% "chmod +x /tmp/%GITPRJ%/dist/usr/local/bin/* ; chmod 755 /tmp/%GITPRJ%/dist/usr/local/bin/* ; cp /tmp/%GITPRJ%/dist/usr/local/bin/restartwsl /tmp/%GITPRJ%/dist/etc/skel/.config/plasma-workspace/shutdown/restartwsl ; chmod -R 700 /tmp/%GITPRJ%/dist/etc/skel/.config ; chmod -R 7700 /tmp/%GITPRJ%/dist/etc/skel/.local ; chmod -R 7700 /tmp/%GITPRJ%/dist/etc/skel/.cache ; chmod 700 /tmp/%GITPRJ%/dist/etc/skel/.mozilla"
%GO% "chmod 755 /tmp/%GITPRJ%/dist/etc/profile.d/kWSL.sh ; chmod +x /tmp/%GITPRJ%/dist/etc/profile.d/kWSL.sh ; chmod 755 /tmp/%GITPRJ%/dist/etc/xrdp/startwm.sh ; chmod +x /tmp/%GITPRJ%/dist/etc/xrdp/startwm.sh"
%GO% "unamestr=`uname -r` ; if [[ "$unamestr" == '4.4.0-17763-Microsoft' ]]; then apt-get purge -y plasma-discover ; sed -i 's/discover/muon/g' /tmp/%GITPRJ%/dist/etc/skel/.config/plasma-org.kde.plasma.desktop-appletsrc ; ln -s /usr/bin/software-properties-qt /usr/bin/software-properties-kde ; fi" > NUL
%GO% "cp -Rp /tmp/%GITPRJ%/dist/* / ; cp -Rp /tmp/%GITPRJ%/dist/etc/skel/.cache /root ; cp -Rp /tmp/%GITPRJ%/dist/etc/skel/.config /root ; cp -Rp /tmp/%GITPRJ%/dist/etc/skel/.local /root"
START /MIN /WAIT "Updates for WSL1 Compatibility" "%DISTROFULL%\LxRunOffline.exe" "r" "-n" "%DISTRO%" "-c" "dpkg -i /tmp/%GITPRJ%/deb/libkf5activitiesstats*.deb /tmp/%GITPRJ%/deb/kactivitymanagerd*.deb /tmp/%GITPRJ%/deb/kinfocenter*.deb /tmp/%GITPRJ%/deb/klassy*.deb"
START /MIN /WAIT "DBUS WSL1 Packages" "%DISTROFULL%\LxRunOffline.exe" "r" "-n" "%DISTRO%" "-c" "dpkg --purge --force-all dbus dbus-x11 libdbus-1-3 dbus-user-session ; dpkg -i --force-all /tmp/%GITPRJ%/deb-dbus/libdbus-1-3_*.deb /tmp/%GITPRJ%/deb-dbus/dbus_*.deb /tmp/%GITPRJ%/deb-dbus/dbus-x11_*.deb /tmp/%GITPRJ%/deb-dbus/xdg-desktop-portal_*.deb /tmp/%GITPRJ%/deb-dbus/libdconf1_*.deb"
...
%GO% "echo 'session bpp:i:32' >> /tmp/%GITPRJ%/kWSL.rdp"
%GO% "echo 'allow desktop composition:i:1' >> /tmp/%GITPRJ%/kWSL.rdp"
%GO% "echo 'connection type:i:6' >> /tmp/%GITPRJ%/kWSL.rdp"
%GO% "echo 'networkautodetect:i:0' >> /tmp/%GITPRJ%/kWSL.rdp"
%GO% "echo 'bandwidthautodetect:i:1' >> /tmp/%GITPRJ%/kWSL.rdp"
%GO% "echo 'full address:s:COMPY:RDPPRT' >> /tmp/%GITPRJ%/kWSL.rdp"
%GO% "echo 'promptcredentialonce:i:1' >> /tmp/%GITPRJ%/kWSL.rdp"
%GO% "echo 'username:s:PLACEHOLDER' >> /tmp/%GITPRJ%/kWSL.rdp"
%GO% "echo 'password 51:b:' >> /tmp/%GITPRJ%/kWSL.rdp"
%GO% "sed -i 's/PLACEHOLDER/%XU%/g' /tmp/%GITPRJ%/kWSL.rdp"
%GO% "sed -i 's/COMPY/LocalHost/g' /tmp/%GITPRJ%/kWSL.rdp"
%GO% "sed -i 's/RDPPRT/%RDPPRT%/g' /tmp/%GITPRJ%/kWSL.rdp"
%GO% "cp /tmp/%GITPRJ%/kWSL.rdp ./kWSL._"
