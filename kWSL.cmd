...
%GO% "echo 'add-apt-repository -y ppa:apt-fast/stable'"
%GO% "rm -rf /etc/apt/apt.conf.d/20snapd.conf /etc/rc2.d/S01whoopsie /etc/init.d/console-setup.sh" 
...
:apt-fast-bionic
START /MIN /WAIT "apt-fast" %GO% "DEBIAN_FRONTEND=noninteractive dpkg -i /tmp/KDE-Neon-For-WSL/deb/aria2_1.35.0-1build1_amd64.deb /tmp/KDE-Neon-For-WSL/deb/libaria2-0_1.35.0-1build1_amd64.deb /tmp/KDE-Neon-For-WSL/deb/libssh2-1_1.8.0-2.1build1_amd64.deb /tmp/KDE-Neon-For-WSL/deb/libc-ares2_1.15.0-1build1_amd64.deb ; chmod +x /tmp/KDE-Neon-For-WSL/dist/usr/local/bin/apt-fast ; cp -p /tmp/KDE-Neon-For-WSL/dist/usr/local/bin/apt-fast /usr/local/bin" > NUL
...
%GO% "sed -i 's#SCHT#%SCHT%#g' /tmp/KDE-Neon-For-WSL/dist/usr/local/bin/restartwsl ; sed -i 's#DISTRO#%DISTRO%#g' /tmp/KDE-Neon-For-WSL/dist/usr/local/bin/restartwsl"
%GO% "sed -i 's/QQQ/%WINDPI%/g' /tmp/KDE-Neon-For-WSL/dist/etc/skel/.config/kdeglobals"
%GO% "sed -i 's/QQQ/%LINDPI%/g' /tmp/KDE-Neon-For-WSL/dist/etc/skel/.config/kcmfonts"
%GO% "sed -i 's/KPANEL/%KPANEL%/g' /tmp/KDE-Neon-For-WSL/dist/etc/skel/.config/plasmashellrc"
...
%GO% "sed -i 's/port=3389/port=%RDPPRT%/g' /tmp/KDE-Neon-For-WSL/dist/etc/xrdp/xrdp.ini ; cp /tmp/KDE-Neon-For-WSL/dist/etc/xrdp/xrdp.ini /etc/xrdp/xrdp.ini"
...
%GO% "sed -i 's/WSLINSTANCENAME/%DISTRO%/g' /tmp/KDE-Neon-For-WSL/dist/usr/local/bin/initwsl"
%GO% "sed -i 's/\\h/%DISTRO%/g' /tmp/KDE-Neon-For-WSL/dist/etc/skel/.bashrc ; ln -s /usr/lib/x86_64-linux-gnu/libexec/kf5/kdesu /usr/bin/kdesu ; rm /usr/lib/x86_64-linux-gnu/qt5/plugins/discover/fwupd-backend.so"
...
%GO% "chmod 644 /tmp/KDE-Neon-For-WSL/dist/etc/wsl.conf ; chmod 644 /tmp/KDE-Neon-For-WSL/dist/var/lib/xrdp-pulseaudio-installer/*.so"
%GO% "chmod +x /tmp/KDE-Neon-For-WSL/dist/usr/local/bin/* ; chmod 755 /tmp/KDE-Neon-For-WSL/dist/usr/local/bin/* ; cp /tmp/KDE-Neon-For-WSL/dist/usr/local/bin/restartwsl /tmp/KDE-Neon-For-WSL/dist/etc/skel/.config/plasma-workspace/shutdown/restartwsl ; chmod -R 700 /tmp/KDE-Neon-For-WSL/dist/etc/skel/.config ; chmod -R 7700 /tmp/KDE-Neon-For-WSL/dist/etc/skel/.local ; chmod -R 7700 /tmp/KDE-Neon-For-WSL/dist/etc/skel/.cache ; chmod 700 /tmp/KDE-Neon-For-WSL/dist/etc/skel/.mozilla"
%GO% "chmod 755 /tmp/KDE-Neon-For-WSL/dist/etc/profile.d/kWSL.sh ; chmod +x /tmp/KDE-Neon-For-WSL/dist/etc/profile.d/kWSL.sh ; chmod 755 /tmp/KDE-Neon-For-WSL/dist/etc/xrdp/startwm.sh ; chmod +x /tmp/KDE-Neon-For-WSL/dist/etc/xrdp/startwm.sh"
%GO% "unamestr=`uname -r` ; if [[ "$unamestr" == '4.4.0-17763-Microsoft' ]]; then apt-get purge -y plasma-discover ; sed -i 's/discover/muon/g' /tmp/KDE-Neon-For-WSL/dist/etc/skel/.config/plasma-org.kde.plasma.desktop-appletsrc ; ln -s /usr/bin/software-properties-qt /usr/bin/software-properties-kde ; fi" > NUL
%GO% "cp -Rp /tmp/KDE-Neon-For-WSL/dist/* / ; cp -Rp /tmp/KDE-Neon-For-WSL/dist/etc/skel/.cache /root ; cp -Rp /tmp/KDE-Neon-For-WSL/dist/etc/skel/.config /root ; cp -Rp /tmp/KDE-Neon-For-WSL/dist/etc/skel/.local /root"
START /MIN /WAIT "Updates for WSL1 Compatibility" "%DISTROFULL%\LxRunOffline.exe" "r" "-n" "%DISTRO%" "-c" "dpkg -i /tmp/KDE-Neon-For-WSL/deb/libkf5activitiesstats*.deb /tmp/KDE-Neon-For-WSL/deb/kactivitymanagerd*.deb /tmp/KDE-Neon-For-WSL/deb/kinfocenter*.deb /tmp/KDE-Neon-For-WSL/deb/klassy*.deb"
START /MIN /WAIT "DBUS WSL1 Packages" "%DISTROFULL%\LxRunOffline.exe" "r" "-n" "%DISTRO%" "-c" "dpkg --purge --force-all dbus dbus-x11 libdbus-1-3 dbus-user-session ; dpkg -i --force-all /tmp/KDE-Neon-For-WSL/deb-dbus/libdbus-1-3_*.deb /tmp/KDE-Neon-For-WSL/deb-dbus/dbus_*.deb /tmp/KDE-Neon-For-WSL/deb-dbus/dbus-x11_*.deb /tmp/KDE-Neon-For-WSL/deb-dbus/xdg-desktop-portal_*.deb /tmp/KDE-Neon-For-WSL/deb-dbus/libdconf1_*.deb"
...
%GO% "echo 'session bpp:i:32' >> /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "echo 'allow desktop composition:i:1' >> /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "echo 'connection type:i:6' >> /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "echo 'networkautodetect:i:0' >> /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "echo 'bandwidthautodetect:i:1' >> /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "echo 'full address:s:COMPY:RDPPRT' >> /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "echo 'promptcredentialonce:i:1' >> /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "echo 'username:s:PLACEHOLDER' >> /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "echo 'password 51:b:' >> /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "sed -i 's/PLACEHOLDER/%XU%/g' /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "sed -i 's/COMPY/LocalHost/g' /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "sed -i 's/RDPPRT/%RDPPRT%/g' /tmp/KDE-Neon-For-WSL/kWSL.rdp"
%GO% "cp /tmp/KDE-Neon-For-WSL/kWSL.rdp ./kWSL._"
